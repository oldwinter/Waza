...
- 会替换用户现有文件的 installer script，必须先把下载内容写入同目录的临时文件，并通过同一个 `EXIT`/`INT`/`TERM` trap 清理，确保 transfer 失败或中断都不会留下部分安装内容。`setup-rule.sh` 和 `setup-statusline.sh` 必须保持这种结构一致；修改其中一个时同时检查另一个，并为每项 guard 提供独立的失败测试。
- Report-only launcher 必须在第一次执行前 canonicalize 每一个 inherited 或 discovered executable candidate（包括它的 reparse target），并拒绝位于 audited project 内的 candidate。每个 discovery source 都要有 platform test coverage。
