...
| 18 | Mishandle a bundle or terminal goal | 没读完消息就对第一句动手；消息包含多个请求或 screenshots 时静默丢掉部分；把每项都当成 to-do；或在明确授权的最终状态之前于内部检查点交回控制权 | 先读完整条消息，枚举并分类每个 ask，锁定 accepted completion ledger，并持续推进到授权终点。只有到达请求的终点、真实 blocker 或缺少必要授权时才停止；明确报告 deferred items |
