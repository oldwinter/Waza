...
5. 命名 exact boundary：already released、fixed on `main` but unreleased、available in nightly/beta/preview、next release、not planned、duplicate，或 still needs evidence。除非当前 turn 已检查该状态，不要写 "shipped"、"released" 或 "verified"。
...
7. 更新 wording 时，优先 **editing** existing maintainer comment（`PATCH /repos/{owner}/{repo}/issues/comments/{comment_id}`）；除非 old text 必须从 history 消失，否则避免 delete plus repost。
