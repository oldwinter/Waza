...
> **中文导读（下方英文为 canonical contract）：** `ui` 的目标是可运行且有明确观点的 visual surface。除 screenshot、viewport、localized text-fit 外，还要覆盖 generated artifact 的真实输出；U+2014 em dash 是禁止输出字符。截图迭代要冻结 target、preserve、evidence 和必要的 before/during/after 状态矩阵。
