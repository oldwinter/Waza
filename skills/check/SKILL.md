...
> **中文导读（下方英文为 canonical contract）：** `check` 默认只生成 review、audit、triage 和 release-readiness 报告；只有当前 turn 明确授权 fix、change、implement 或 optimize 才能写入。把 modified、staged 和 untracked files 当成用户工作，除非获得明确授权，不得移动、隐藏、覆盖、清理或丢弃。Ship 链必须持续完成 completion ledger，逐项核对 generated artifact、package、remote 和 public action 状态；修复后要重新冻结 baseline、复跑暴露问题的 check，并重新完成 review。保留下方命令、字段和 severity contract。
...
- **带着本轮未处理 finding 发布**：如果同一次运行先产生 review findings，随后进入 ship action，必须先修复每条 finding，或把它重述为“已知，仍然发布”，说明 user impact 并再次确认，才能继续 release。已有的长期 release 授权不覆盖授权之后才发现的问题。
