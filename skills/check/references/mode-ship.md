...
5. 只有用户明确批准后，才能 push、publish、tag 或创建 release。用户指定 exact account 时，在第一次 remote write 前立即验证已认证的 service identity；如果不匹配就停止，绝不静默替换为其他 account。若 auth、OTP、CI、registry 或 network 状态阻塞操作，暂停并报告 exact blocker。
