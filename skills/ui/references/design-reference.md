...
> **中文导读（下方英文为 canonical contract）：** 先固定项目的 CSS strategy 和 visual stack，避免相互冲突的 framework、animation 或 icon system。新增规则要继续区分产品 UI、文档输出、generated asset、motion 与 data visualization；`audit prompts` 是 review evidence，不自动扩大 implementation scope。
