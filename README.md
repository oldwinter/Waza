...
这会从 repo marketplace 把 Waza 安装为 Codex plugin；后续更新可以运行 `codex plugin marketplace upgrade waza`，再用 `codex plugin add waza@waza` 刷新已安装的 plugin snapshot。
如果你更喜欢 legacy skills installer，可使用 `npx skills add tw93/Waza -a codex -g -y`；只安装单个 skill 可使用 `npx skills add tw93/Waza --skill think -a codex -g -y`。Codex sessions 可以按名称调用已安装的 skills，也可以链接到 `npx skills path tw93/Waza` 显示的已安装 `SKILL.md` 路径。
...
Codex plugin 安装使用 `codex plugin marketplace upgrade waza`，然后运行 `codex plugin add waza@waza` 刷新已安装的 plugin snapshot。
